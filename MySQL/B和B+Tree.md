...
对于数据在磁盘等辅助存储设别的情况，红黑树并不擅长，因为红黑树还是过高，当数据在磁盘中，磁盘IO会成为最大的性能瓶颈，设计的目标应该是尽量减少IO次数，而树的高度越高，crud所需要的IO次数页越多，严重影响性能。因为IO操作效率低，当大量数据存储时，查询不能一下子将所有数据加载到内存中，只能逐一加载磁盘页，每个磁盘页对应树的结点
